...
- [x] 1.3 Window is **resizable**; drag the divider between panes — both panes stay usable; the window has a sensible minimum size.
- [x] 1.4 There is **no "New Window"** item in the File/Window menus; ⌘N does nothing (single-window app).
- [x] 1.5 Resize/move the window, **Quit (⌘Q)**, relaunch → window **size/position is restored**.
...
- [x] 3.1a Enter **Email only** → the Link text field shows the email as **ghost text** (live-updating as you type the email) and a snippet **is generated** using the email as the link text. Tab into Link text and press **Tab** → the field **fills in** with the email, focus stays in the field with the insertion point at the end; a second Tab moves on.
...
- [x] 4.6 Valid email, **Link text empty** → **exception:** a snippet IS generated (link text defaults to the email; see 3.1a).
- [x] 4.7 Valid email, Link text = `"   "` (spaces only) → same as 4.6 (whitespace counts as empty).
...
- [x] 5.1 An **`info.circle`** icon sits beside the field's label (same
...
- [x] 8.5 **▸ Show decoded source** expands to show the decoded HTML; it equals your input. Collapse it. The **View ▸ Show/Hide Decoded Source** menu item toggles the same disclosure; its title reflects the current state (Show ↔ Hide).
...
- [x] 9.3 **Click the link** → it does **not** open Mail; a transient **"Preview is non-interactive"** toast appears at the click point (~2s); no other UI moves or resizes.
...
- [x] 10.5 **Saved Values ▾ → Manage Saved Values…** → rename an entry (edit the name in place, press Return), delete one (trash appears on hover; also in the row's context menu), and **drag a row's gripper to reorder** — the lifted row follows the pointer and the others animate out of the way → the menu reflects all changes.
...
- [x] 12.3 **View** menu: **Basic (⌘1)**, **Advanced (⌘2)**, **Show/Hide Decoded Source** (title changes with the disclosure state).
...
- [x] 13.3 **No-JavaScript fallback message** field: type text → reflected in the snippet's `<span>` fallback (check Show decoded source / the snippet). Type an **`@`** → it is **stripped** (can't be entered), preserving the no-`@` guarantee.
- [x] 13.4 Change both settings, **Quit and relaunch** → settings **persist**.
...
- [ ] **Exit codes:** `obfuskode` alone on a TTY prints usage and exits 64
      ...
      (`echo $?`).
- [x] **Help window:** Help ▸ Obfuskoder CLI Help opens the small window;
